...
void draw_path(Mat& image, Mat& mask, Node** nodes, Point seed, bool flag, double scale, bool is_debug)
{
	int thickness = 2;
	int lineType = 8;
	...
	while(tem.pre[0] != -1)
	{
		...
		tem = nodes[tem.pre[0]][tem.pre[1]];
	}
}

void draw_path_tree(Mat& tree_graph, Mat& cost_graph, int node_num, 
				  Node** nodes, Point seed,  
...
void stop(Mat& tem1, Mat& o_image, Mat& mask, double scale, 
		  ...
		  stack<Mat> mask_stack, Node** nodes, 
